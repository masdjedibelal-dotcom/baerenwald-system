...
async function loadAufschubMap(): Promise<Map<string, string>> {
  ...
  if (error) logDbError('lib/datenschutz/queries:datenschutz_aufschub', error)
...
export async function loadDatenschutzFristen(): Promise<DatenschutzFristRow[]> {
  ...
  if (error) logDbError('lib/datenschutz/queries:datenschutz_fristen', error)
...
export async function loadDatenschutzLog(limit = 200): Promise<DatenschutzLoeschlogRow[]> {
  const { data, error } = await supabaseAdmin
    ...
    .limit(limit)
  if (error) logDbError('lib/datenschutz/queries:datenschutz_loeschlog', error)
  if (error || !data) return []
...
export async function loadDatenschutzAnfragen(): Promise<DatenschutzAnfrageRow[]> {
  ...
  if (error) logDbError('lib/datenschutz/queries:datenschutz_anfragen', error)
  if (error || !data) return []
...
async function loadLeadIdsMitAuftrag(): Promise<Set<string>> {
  const { data, error } = await supabaseAdmin
    ...
    .not('lead_id', 'is', null)
  if (error) logDbError('lib/datenschutz/queries:angebote', error)
  const ids = new Set<string>()
  for (const row of data ?? []) {
    const lid = (row as { lead_id: string | null }).lead_id
    if (lid) ids.add(lid)
  }
  const { data: leadsAuftrag, error: error2 } = await supabaseAdmin.from('leads').select('id').eq('status', 'auftrag')
  if (error2) logDbError('lib/datenschutz/queries:leads', error2)
...
export async function loadDatenschutzFaellige(): Promise<DatenschutzFaelligRow[]> {
  ...
  if (mFotos != null) {
    ...
    if (error) logDbError('lib/datenschutz/queries:auftraege', error)

    const ids = (auf ?? []).map((r) => String((r as { id: string }).id))
    if (ids.length) {
      const { data: fe, error } = await supabaseAdmin.from('formular_eintraege').select('auftrag_id, foto_urls').in('auftrag_id', ids)
      if (error) logDbError('lib/datenschutz/queries:formular_eintraege', error)
  ...
  if (mLeadAbs != null) {
    ...
    const { data: leads2, error } = await supabaseAdmin
      .from('leads')
      .select('id, kontakt_name, updated_at')
      .eq('status', 'abgelehnt')
      .lte('updated_at', cutoffEnd)
  ...
  if (mKd != null) {
    ...
    const { data: kunden, error } = await supabaseAdmin.from('kunden').select('id, name, created_at')
    if (error) logDbError('lib/datenschutz/queries:kunden', error)
    ...
    for (const k of kunden ?? []) {
      ...
      const {count: rCount, error } = await supabaseAdmin
        ...
        .eq('kunde_id', kid)
      if (error) logDbError('lib/datenschutz/queries:rechnungen', error)
      if ((rCount ?? 0) > 0) continue

      const {count: aCount, error: error2 } = await supabaseAdmin
        .from('auftraege')
        .select('id', { count: 'exact', head: true })
        .eq('kunde_id', kid)
      if (error2) logDbError('lib/datenschutz/queries:auftraege', error2)
      if ((aCount ?? 0) > 0) continue

      const { data: lastLead, error: error3 } = await supabaseAdmin
        .from('leads')
        .select('updated_at')
        .eq('kunde_id', kid)
        ...
        .limit(1)
        .maybeSingle()
      if (error3) logDbError('lib/datenschutz/queries:leads', error3)
  ...
  if (mMelderFotos != null) {
    ...
    const { data: melderLeadsFotos, error } = await supabaseAdmin
      .from('leads')
  ...
  if (mMelderOffen != null) {
    ...
    const { data: melderOffen, error } = await supabaseAdmin
      .from('leads')
      .select(
  ...
  if (mMelderAb != null) {
    ...
    const { data: melderAb, error } = await supabaseAdmin
      .from('leads')
      .select(
