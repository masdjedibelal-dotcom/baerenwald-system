...
export async function executeDatenschutzLoeschung(input: {
  ...
  if (kategorie === 'einheit_bewohner') {
    const { data: row } = await supabaseAdmin
      .from('einheit_bewohner')
      .select('id, kunde_id, name, anonymisiert_am')
      .eq('id', referenz_id)
      .maybeSingle()
    if (!row) return { ok: false, message: 'Bewohner nicht gefunden' }
    if ((row as { anonymisiert_am?: string | null }).anonymisiert_am) {
      return { ok: false, message: 'Bewohner bereits anonymisiert.' }
    }
    const kid = String((row as { kunde_id: string }).kunde_id)
    const { error } = await supabaseAdmin
      .from('einheit_bewohner')
      .update({
        name: 'Anonymisiert',
        ...
        email: null,
        aktiv: false,
        anonymisiert_am: new Date().toISOString(),
    ...
    if (error) return { ok: false, message: error.message }
    await insertLoeschlog({
      typ: 'einheit_bewohner',
      referenz_id,
      referenz_typ: 'einheit_bewohner',
      grund,
      geloescht_von: userId,
    })
    return { ok: true }
