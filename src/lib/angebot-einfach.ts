...
export function resolveStatusEinfach(
  row: Pick<Angebot, 'status' | 'status_einfach' | 'gueltig_bis'>
): AngebotStatusEinfach {
  const raw = row.status_einfach?.trim()
  if (raw === 'entwurf' || raw === 'gesendet' || raw === 'angenommen' || raw === 'abgelehnt') {
...
export function kundeNameAusAngebot(a: {
  ...
  const kontakt = a.leads?.kontakt_name?.trim()
