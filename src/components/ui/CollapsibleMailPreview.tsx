...
export function CollapsibleMailPreview({
  ...
  return (
    <div className="rounded-button border border-bw-border bg-white">
      <MockBtn fullWidth className="flex min-h-[44px] items-center justify-between gap-2 px-3 py-2.5 text-left md:cursor-default" type="button" onClick={() => setUserToggled((v) => !(v ?? !isMobile))} aria-expanded={expanded}>
      ...
      {expanded ? (
        <div className="border-t border-bw-border p-2 md:border-t-0 md:pt-0">
          <iframe
            ...
            sandbox=""
            className="h-[min(280px,40vh)] w-full rounded-card border-0 bg-white md:h-[280px]"
