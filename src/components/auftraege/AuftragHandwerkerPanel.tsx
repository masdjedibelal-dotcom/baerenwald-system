...
import { FileText, HardHat, UserPlus } from 'lucide-react'
...
export function AuftragHandwerkerPanel({
  ...
  const gruppen = useMemo((): GewerkGruppe[] => {
    ...
    for (const g of gewerke) {
      byGewerk.set(g.id, {
        ...
        gewerkName: g.name,
        gewerkSlug: g.slug,
        zuweisung: handwerkerRows.find((z) => z.gewerk_id === g.id) ?? null,
        positionen: [],
      })
    }
    ...
    for (const p of positionen) {
      ...
      if (!byGewerk.has(key)) {
        byGewerk.set(key, {
          ...
          zuweisung: g ? (handwerkerRows.find((z) => z.gewerk_id === g.id) ?? null) : null,
          positionen: [],
        })
      }
  ...
  function openGewerkModal(gruppe: GewerkGruppe) {
    if (!gruppe.gewerkId) {
