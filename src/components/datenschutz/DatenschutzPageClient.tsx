...
import { MockBtn, MockTable, MockEmpty } from '@/components/mock-ui'
...
export function DatenschutzPageClient({ fristen, faellig, log, anfragen, vvt }: Props) {
  ...
  async function saveFrist() {
    ...
    if (!Number.isFinite(n) || n < 1) {
      applyFieldErrors({ _form: TOAST.bitte_gueltige_monatszahl_eingeben })
      return
    }
    setBusy('frist')
    ...
    toast.success(TOAST.frist_gespeichert)
    setFristModal(null)
  ...
  async function runAufschub() {
    ...
    toast.success(TOAST.um_12_monate_zurueckgestellt)
  ...
  async function runCreateAnfrage() {
    if (!anfrageName.trim() || !anfrageEmail.trim()) {
      toast.error(TOAST.name_und_e_mail_erforderlich)
      return
    }
    setBusy('anfrage')
    ...
    toast.success(TOAST.anfrage_erfasst)
  ...
  async function runMelderSuche() {
    if (!melderSucheEmail.trim()) {
      toast.error(TOAST.e_mail_eingeben)
      return
    }
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap gap-2 border-b border-border pb-2">
        {(
          ...
          <MockBtn className={mainTab === id
                ? 'rounded-button border border-bw-primary bg-bw-green-bg px-3 py-1.5 text-sm font-medium text-bw-primary'
                : 'rounded-button px-3 py-1.5 text-sm text-muted hover:bg-canvas'} key={id} type="button" onClick={() => setMainTab(id)}>
            {label}
          </MockBtn>
        ))}
      </div>

      <div className="rounded-button border border-status-new-bg bg-status-new-bg px-4 py-3 text-sm text-status-new-text">
      ...
      <details className="rounded-card border border-border bg-canvas/40 px-4 py-3 text-sm text-ink">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-muted">
          ...
          <li>Screenshots/Fotos nicht außerhalb des Systems speichern.</li>
      ...
      <section>
        ...
        <Card className="overflow-x-auto p-0">
          <MockTable className="w-full min-w-[640px] border-collapse text-left text-sm">
            <thead>
            ...
            <tbody>
              {fristen.map((f) => (
                <tr key={f.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-3 py-2">
                    <MockBtn className={`rounded-pill px-3 py-1 text-xs font-medium ${
                        f.aktiv ? 'bg-status-order-bg text-status-order-text' : 'bg-muted/30 text-muted'
                      }`} type="button" disabled={busy === `aktiv-${f.id}`} onClick={() => void toggleAktiv(f)}>
                      {f.aktiv ? 'Aktiv' : 'Inaktiv'}
                    </MockBtn>
                  </td>
                  <td className="px-3 py-2 text-right">
                    <MockBtn type="button" kind="secondary" sm onClick={() => openFrist(f)}>
                      Frist bearbeiten
                    </MockBtn>
                  </td>
                </tr>
      ...
      <section>
        ...
        {faellig.length === 0 ? (
          <div className="rounded-card border border-status-order-bg bg-status-order-bg px-4 py-3 text-sm text-status-order-text">
            Keine fälligen Löschungen
          </div>
        ) : (
          <ul className="space-y-3">
            {faellig.map((row) => (
              <li key={`${row.kategorie}-${row.referenz_id}`} className="rounded-card border border-border bg-surface p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <span className="mb-1 inline-block rounded-card bg-canvas px-2 py-0.5 text-xs font-medium text-ink">
                  ...
                  <div className="flex shrink-0 flex-wrap gap-2">
                    <MockBtn type="button" kind="danger" sm onClick={() => setDelModal(row)}>
                      Jetzt löschen
                    </MockBtn>
                    <MockBtn type="button" kind="secondary" sm onClick={() => setAufschubModal(row)}>
                      Zurückstellen (+12 M.)
                    </MockBtn>
                  </div>
                </div>
      ...
      <section>
        ...
        <div className="mb-3">
          <a
            ...
            className="inline-flex min-h-[44px] items-center rounded-card border border-border bg-surface px-4 text-sm font-medium text-ink hover:bg-canvas"
          >
            Löschprotokoll exportieren (CSV)
          </a>
        </div>
        <Card className="max-h-[360px] overflow-auto p-0">
          <MockTable className="w-full min-w-[560px] border-collapse text-left text-xs">
      ...
      <section className="space-y-6">
        <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <MockBtn type="button" kind="primary" sm onClick={() => setAnfrageOpen(true)}>
            + Anfrage erfassen
          </MockBtn>
        </div>
        ...
        <Card className="p-4">
          ...
          <div className="flex flex-wrap gap-2">
            <MockField label="Melder-E-Mail"><MockInput type="email" name="melder_suche" value={melderSucheEmail} onChange={(e) => setMelderSucheEmail(e.target.value)} className="min-w-[220px] flex-1" /></MockField>
            <div className="flex items-end">
              <MockBtn type="button" kind="secondary" sm loading={busy === 'melder-suche'} onClick={() => void runMelderSuche()}>
                Suchen
              </MockBtn>
            </div>
          </div>
          {melderSucheLief ? (
            melderTreffer.length === 0 ? (
              <MockEmpty title="Keine Melder-Leads gefunden." />
            ) : (
              <ul className="mt-3 space-y-2">
                {melderTreffer.map((l) => (
                  <li key={l.id} className="flex flex-wrap items-center justify-between gap-2 rounded-card border border-border px-3 py-2 text-sm">
                    <div>
        ...
        {offeneAnfragen.map((a) => {
          ...
          return (
            <div
              key={a.id}
              className={`mb-2 rounded-card border px-3 py-2 text-sm ${warn ? 'border-status-contact-bg bg-status-contact-bg text-status-contact-text' : 'border-border bg-surface'}`}
            >
              {warn ? (
        ...
        {anfragen.filter((a) => a.status === 'erledigt').length > 0 ? (
          <details className="mt-4 rounded-card border border-border bg-canvas/30 p-3 text-sm">
      ...
      {fieldErrors._form ? <p className="field-error" role="alert">{fieldErrors._form}</p> : null}
          ...
          <>
            ...
            <MockField label="Frist (Monate)"><MockInput type="number" name="frist_monate" min={1} value={fristMonate} onChange={(e) => setFristMonate(e.target.value)} /></MockField>
            <label className="mt-3 flex items-center gap-2 text-sm text-ink">
              <MockCheckbox checked={fristAktiv} onChange={(e) => setFristAktiv(e.target.checked)} />
            ...
            <div className="mt-4 flex gap-2">
              <MockBtn type="button" kind="secondary" onClick={() => setFristModal(null)}>
                Abbrechen
              </MockBtn>
              <MockBtn type="button" kind="primary" loading={busy === 'frist'} onClick={() => void saveFrist()}>
                Speichern
              </MockBtn>
            </div>
          </>
      ...
      <ConfirmPopup
        open={!!delModal}
        onClose={() => {
          if (busy !== 'del') setDelModal(null)
        }}
        title="Löschung bestätigen"
        danger
        busy={busy === 'del'}
        confirmLabel={busy === 'del' ? 'Wird gelöscht…' : 'Endgültig ausführen'}
...
>>>>>>> Stashed changes
        ...
        {aufschubModal ? (
          <>
            ...
            <label className="block text-sm">
              ...
              <RichTextEditor value={typeof (aufschubText) === 'string' ? (aufschubText) : ''} onChange={(__v) => setAufschubText(__v)} minHeight={120} />
            </label>
            <div className="mt-3 flex gap-2">
              <MockBtn type="button" kind="secondary" onClick={() => setAufschubModal(null)}>
                Abbrechen
              </MockBtn>
              <MockBtn type="button" kind="primary" loading={busy === 'aufschub'} onClick={() => void runAufschub()}>
                Speichern
              </MockBtn>
            </div>
          </>
      ...
      <EditorSheet open={anfrageOpen} onClose={() => setAnfrageOpen(false)} title="Anfrage erfassen" size="md">
        <div className="max-h-[60vh] space-y-3 overflow-y-auto">
          <label className="block text-sm font-medium text-ink">
            Typ
            <MockSelect className="mt-1 w-full min-h-[44px] rounded-card border border-border bg-surface px-3 text-ink" value={anfrageTyp} onChange={(e) => setAnfrageTyp(e.target.value as typeof anfrageTyp)}>
          ...
          <MockField label="Name"><MockInput name="an_name" value={anfrageName} onChange={(e) => setAnfrageName(e.target.value)} /></MockField>
          <MockField label="E-Mail"><MockInput type="email" name="an_email" value={anfrageEmail} onChange={(e) => setAnfrageEmail(e.target.value)} /></MockField>
          <label className="block text-sm font-medium text-ink">
            Kontext
            <MockSelect className="mt-1 w-full min-h-[44px] rounded-card border border-border bg-surface px-3 text-ink" value={anfrageKontext} onChange={(e) => setAnfrageKontext(e.target.value as DatenschutzAnfrageKontext)}>
          ...
          <label className="block text-sm">
            ...
            <RichTextEditor value={typeof (anfrageDesc) === 'string' ? (anfrageDesc) : ''} onChange={(__v) => setAnfrageDesc(__v)} minHeight={120} />
          </label>
        </div>
        <div className="mt-4 flex gap-2">
          <MockBtn type="button" kind="secondary" onClick={() => setAnfrageOpen(false)}>
            Abbrechen
          </MockBtn>
          <MockBtn type="button" kind="primary" loading={busy === 'anfrage'} onClick={() => void runCreateAnfrage()}>
            Speichern
          </MockBtn>
        </div>
      </EditorSheet>

      <EditorSheet open={!!editAnfrage} onClose={() => setEditAnfrage(null)} title="Anfrage bearbeiten" size="lg">
        {editAnfrage ? (
          <>
            <p className="mb-3 text-sm text-muted">
            ...
            <div className="max-h-[50vh] space-y-3 overflow-y-auto">
              <label className="block text-sm">
                ...
                <RichTextEditor value={typeof (editNotizen) === 'string' ? (editNotizen) : ''} onChange={(__v) => setEditNotizen(__v)} minHeight={120} />
              </label>
              <label className="block text-sm font-medium text-ink">
                Status
                <MockSelect className="mt-1 w-full min-h-[44px] rounded-card border border-border bg-surface px-3" value={editStatus} onChange={(e) => setEditStatus(e.target.value as typeof editStatus)}>
            ...
            <div className="mt-4 flex gap-2">
              <MockBtn type="button" kind="secondary" onClick={() => setEditAnfrage(null)}>
                Abbrechen
              </MockBtn>
              <MockBtn type="button" kind="primary" loading={busy === 'edit-anfrage'} onClick={() => void runSaveAnfrage()}>
                Speichern
              </MockBtn>
            </div>
          </>
