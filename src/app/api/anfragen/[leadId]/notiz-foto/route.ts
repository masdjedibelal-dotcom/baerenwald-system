...
export async function POST(req: Request, { params }: { params: { leadId: string } }) {
  ...
  if (error) logDbError('app/api/anfragen/[leadId]/notiz-foto/route:leads', error)
  if (!lead) {
  ...
  if (upErr) logDbError('app/api/anfragen/[leadId]/notiz-foto/route:lead-notizen-fotos', upErr)
