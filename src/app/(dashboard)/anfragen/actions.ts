...
export async function createAnfrage(
  ...
  const melderEinheit = payload.melder_einheit?.trim() || null
...
/** Melder + Leistungsort/Objekt am Lead (CRM-Angebot ohne Anfrage-Wizard). */
export async function updateLeadMelderUndLeistungsort(
  leadId: string,
  data: {
    melder_name?: string | null
    melder_email?: string | null
  ...
  if (data.melder_name !== undefined) patch.melder_name = data.melder_name?.trim() || null
  if (data.melder_email !== undefined) patch.melder_email = data.melder_email?.trim() || null
  if (data.melder_telefon !== undefined) {
    patch.melder_telefon = data.melder_telefon?.trim() || null
  }
  if (data.melder_einheit !== undefined) {
...
/**
 * Soft-delete nur für reine Anfragen ohne Dokumente.
 * Mit Angebot/Auftrag/Rechnung → ablehnen (sonst verschwindet der ganze Vorgang aus Listen).
 * Vorgang bewusst ausblenden: `deleteVorgang` unter /vorgaenge.
 */
export async function softDeleteAnfrage(
  ...
  if ((angCount ?? 0) > 0 || (aufCount ?? 0) > 0) {
    return {
      ok: false,
      message:
        'Diese Anfrage hat bereits Angebot oder Auftrag. Bitte über Vorgänge löschen — nicht hier.',
    }
  }
...
/**
 * Anfrage löschen = Soft-Delete ohne Dokumente (nie Hard-Delete/CASCADE).
 * Alias für softDeleteAnfrage.
 */
export async function deleteAnfrage(
  leadId: string
): Promise<{ ok: true } | { ok: false; message: string }> {
  return softDeleteAnfrage(leadId)
}
