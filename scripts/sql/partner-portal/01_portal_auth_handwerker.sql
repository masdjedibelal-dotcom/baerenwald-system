...
-- Ohne security definer: RLS-Rekursion angebote ↔ leads/kunden (siehe fix-angebote-rls-recursion.sql)
create or replace function public.portal_handwerker_angebot_ids()
returns setof uuid
language sql stable security definer set search_path = public
as $$
  select ah.angebot_id from public.angebot_handwerker ah
...
language sql stable security definer set search_path = public
as $$
  select distinct a.lead_id from public.angebote a
  inner join public.angebot_handwerker ah on ah.angebot_id = a.id
  where ah.handwerker_id = public.portal_handwerker_id() and a.lead_id is not null;
$$;
...
language sql stable security definer set search_path = public
as $$
  select distinct a.kunde_id from public.angebote a
  inner join public.angebot_handwerker ah on ah.angebot_id = a.id
  where ah.handwerker_id = public.portal_handwerker_id() and a.kunde_id is not null;
$$;
