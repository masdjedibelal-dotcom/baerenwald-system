...
export function ProjektStatusClient({
  ...
  useEffect(() => {
    if (!highlightUpdateId || !timeline.some((u) => u.id === highlightUpdateId)) return
    if (!alleUpdates && !updatesFirst.some((u) => u.id === highlightUpdateId)) {
      setAlleUpdates(true)
    }
  }, [highlightUpdateId, timeline, alleUpdates, updatesFirst])
  ...
  }, [highlightUpdateId, alleUpdates, timeline.length])
