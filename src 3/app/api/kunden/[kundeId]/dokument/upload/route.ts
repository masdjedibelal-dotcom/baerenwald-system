...
import { KUNDEN_DOKUMENTE_BUCKET } from '@/lib/kunden/kunden-dokumente-helpers'
...
export async function POST(
  req: Request,
  { params }: { params: Promise<{ kundeId: string }> | { kundeId: string } }
) {
  ...
  const kundeId = resolved.kundeId?.trim()
  if (!kundeId) {
  ...
  const path = `${kundeId}/${Date.now()}-${user.id.slice(0, 8)}-${safe}`
  ...
  if (upErr) {
    ...
    const hint =
      /bucket not found|does not exist/i.test(raw)
        ? ' — Bucket „kunden-dokumente“ fehlt: Migration `20260719120000_kunden_dokumente_storage.sql` ausführen.'
        : ''
    return NextResponse.json({ error: raw + hint }, { status: 500 })
  }

  const { data: pub } = supabaseAdmin.storage.from(KUNDEN_DOKUMENTE_BUCKET).getPublicUrl(path)
