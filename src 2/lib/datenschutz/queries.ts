...
export async function loadDatenschutzFaellige(): Promise<DatenschutzFaelligRow[]> {
  ...
  const mBewohner = fm('einheit_bewohner')
  if (mBewohner != null) {
    const cutoffEnd = `${monthsAgoDateOnly(mBewohner)}T23:59:59.999Z`
    const { data: bewohner } = await supabaseAdmin
      .from('einheit_bewohner')
      .select('id, name, created_at, updated_at, objekt_einheiten(bezeichnung, kunden_objekte(titel))')
      .eq('aktiv', true)
      .is('anonymisiert_am', null)
      .neq('name', 'Anonymisiert')
      .lte('updated_at', cutoffEnd)

    for (const b of bewohner ?? []) {
      const id = String((b as { id: string }).id)
      if (isStillAufgeschoben(aufschub, 'einheit_bewohner', id)) continue
      const u = String((b as { updated_at: string }).updated_at)
      const einheit = (b as { objekt_einheiten?: { bezeichnung?: string; kunden_objekte?: { titel?: string } } | null })
        .objekt_einheiten
      const titel = [
        String((b as { name: string }).name),
        einheit?.bezeichnung,
        einheit?.kunden_objekte?.titel,
      ]
        .filter(Boolean)
        .join(' · ')
      out.push({
        kategorie: 'einheit_bewohner',
        referenz_id: id,
        titel,
        basis_datum: u.slice(0, 10),
        monate_faellig: monthsBetween(u.slice(0, 10)),
        beschreibung: 'Bewohner in Objektakte anonymisieren',
      })
    }
  }
